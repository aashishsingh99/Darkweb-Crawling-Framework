...
res=requests.get('http://192.168.43.208:9200')
es = Elasticsearch([{'host': 'http://192.168.43.208', 'port': '9200'}])
...
"""
			...
			r=requests.post('http://192.168.43.208:9200/myindex/docket',data)
			...
			es.index(index='myindex', ignore=400, doc_type='docket', 
        		, body=json.loads(data))
                        ...
                        print(item)
"""
		    r=requests.post('http://192.168.43.208:9200/myindex/docket',data='{"hu":"iop"}')
	            print(r.status)

		    print(json.dumps(item, indent=4))
		    data1=json.dumps(item)
		    print("i am here")
